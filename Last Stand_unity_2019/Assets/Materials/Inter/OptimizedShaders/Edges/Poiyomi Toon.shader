Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/a090563095e23eb439384aeb18dd6f18"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			)
			{
				...
				{
					v.vertex.xz = lerp(v.vertex.xz, normalize(v.vertex.xz) * (0.2 /*_VertexBarrelWidth*/) + v.vertex.xz * (1.0 /*_VertexBarrelHeight*/), (0.0 /*_VertexBarrelAlpha*/));
				}
				if ((0.0 /*_VertexSphereMode*/))
				{
					v.vertex.xyz = lerp(v.vertex.xyz, normalize(v.vertex.xyz + float4(0,0,0,1).xyz) * (0.0 /*_VertexSphereRadius*/) + v.vertex.xyz * (1.0 /*_VertexSphereHeight*/), (0.637 /*_VertexSphereAlpha*/));
				}
				if ((1.0 /*_VertexTornadoMode*/))
				{
					v.vertex.xz = lerp(v.vertex.xz, float3(v.vertex.xz + float2(cos(_Time.y * (1.0 /*_VertexTornadoSpeed*/) + v.vertex.y * (0.001 /*_VertexTornadoIntensity*/)) * (0.01 /*_VertexTornadoRadius*/), sin(_Time.y * (1.0 /*_VertexTornadoSpeed*/) + v.vertex.y * (0.001 /*_VertexTornadoIntensity*/)) * (0.01 /*_VertexTornadoRadius*/)), v.vertex.y), smoothstep((-50.0 /*_VertexTornadoBaseHeight*/), (50.0 /*_VertexTornadoTopHeight*/), v.vertex.y));
				}
				v.vertex.xyz += mul(unity_WorldToObject, float4(0,0,0,1).xyz + ALWorldTranslation + heightOffset).xyz;
				
				if ((1.0 /*_VertexRoundingEnabled*/))
				{
			...
			{
				float3 parallax = 0;
				for (int j = (20.0 /*_ParallaxInternalIterations*/) - 1; j >= 0; j--)
				{
					float ratio = saturate((float)j / max((20.0 /*_ParallaxInternalIterations*/) - 1, 1));
					float2 parallaxOffset = _Time.y * (float4(0.01,-0.008,1,1) + ratio * float4(0,0,1,1));
					float fade = lerp((1.0 /*_ParallaxInternalMinFade*/), (0.0 /*_ParallaxInternalMaxFade*/), ratio);
					fade = pow(fade, 2.2);
					#if defined(PROP_PARALLAXINTERNALMAP) || !defined(OPTIMIZER_ENABLED)
					float4 parallaxColor = UNITY_SAMPLE_TEX2D_SAMPLER(_ParallaxInternalMap, _MainTex, TRANSFORM_TEX(poiMesh.uv[0], _ParallaxInternalMap) + (lerp((0.0 /*_ParallaxInternalMinDepth*/), (0.1 /*_ParallaxInternalMaxDepth*/), ratio)) * - (poiCam.tangentViewDir.xy / (poiCam.tangentViewDir.z)) + parallaxOffset);
					#else
					float4 parallaxColor = 0;
					#endif
					float3 minColor = poiThemeColor(poiMods, float4(0,1,0,1).rgb, (0.0 /*_ParallaxInternalMinColorThemeIndex*/));
					float3 maxColor = poiThemeColor(poiMods, float4(0,1,0.341709,1).rgb, (0.0 /*_ParallaxInternalMaxColorThemeIndex*/));
					float3 parallaxTint = lerp(minColor, maxColor, ratio);
					float parallaxHeight;
					if ((1.0 /*_ParallaxInternalHeightFromAlpha*/))
					{
					...
					parallaxTint = hueShift(parallaxTint, frac(ratio * (0.0 /*_ParallaxInternalHueShiftPerLevel*/)) * (0.0 /*_ParallaxInternalHueShiftEnabled*/), (0.0 /*_ParallaxInternalHueShiftColorSpace*/), (1.0 /*_ParallaxInternalHueSelectOrShift*/));
					...
					{
						if ((1.0 /*_ParallaxInternalBlendMode*/) == 0) parallax += parallaxTint * parallaxHeight * fade;
						if ((1.0 /*_ParallaxInternalBlendMode*/) == 1) parallax = max(parallax, parallaxTint * parallaxHeight * fade);
					}
				}
				parallax = hueShift(parallax, frac((0.0 /*_ParallaxInternalHueShift*/) + (0.0 /*_ParallaxInternalHueShiftSpeed*/) * _Time.x) * (0.0 /*_ParallaxInternalHueShiftEnabled*/), (0.0 /*_ParallaxInternalHueShiftColorSpace*/), (1.0 /*_ParallaxInternalHueSelectOrShift*/));
				#if defined(PROP_PARALLAXINTERNALMAPMASK) || !defined(OPTIMIZER_ENABLED)
				poiFragData.baseColor.rgb = customBlend(poiFragData.baseColor.rgb, parallax.rgb, (8.0 /*_ParallaxInternalSurfaceBlendMode*/), POI2D_SAMPLER_PAN(_ParallaxInternalMapMask, _MainTex, poiUV(poiMesh.uv[(0.0 /*_ParallaxInternalMapMaskUV*/)], float4(1,1,0,0)), float4(0,0,0,0))[(0.0 /*_ParallaxInternalMapMaskChannel*/)]);
				#else
